...
export class VideoCardComponent implements OnInit, OnDestroy {
  @Input() title: string = '';
  @Input() showRootPath!: string;
  @Input() showPreview: boolean = false;
  @Input() showId: number | undefined;
  @Output() cardClick = new EventEmitter<void>();

  iconImgPath: string = '/video_cover.png';
  ...
  ngOnInit(): void {
    this.videoApiService.getIconBlob(this.showRootPath).subscribe({
      next: (blob) => {
  ...
  ngOnDestroy() {
    // zapobiega memory leak
    if (this.iconImgPath) URL.revokeObjectURL(this.iconImgPath);
  }
}
